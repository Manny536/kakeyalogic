...
```math
\mathrm{DDTL}:x=\text{lattice depth},\qquad \mathrm{DDATL}:x=1+|\Im(s)|.
```
...
```math
\Re(ρ)=\frac12 \quad\Longleftrightarrow\quad z\in\mathbb R.
```
...
```math
A_{Φ,K}:\mathrm{Dom}(A)\subset H_{Φ,K}\to H_{Φ,K}
```
...
```math
\mathrm{Spec}(A_{Φ,K})\subset\mathbb R.
```
